...
from ..state_csv_writer import large_df_to_csvs
import re
...
class singleside_ANA_ABFE_OVERALL(ANA_OVERALL):
    ...
    def plot_deltaU(self, wanted_win_lst, output_file_dir, fraction=1):
        ...
        com_cal_FEP_TOOLS = CAL_FREE_ENERGY(lambda_unk_df, False, 1,True)
        with bf_af_plot(output_file_dir, wanted_win_lst):
            com_cal_FEP_TOOLS.plot_simulation_deltaU(50)

    def posttreat_data(self, _value, output_file_dir, wanted_win_lst, fraction=1):
        lambda_dict = self.com_ANA_FEP_TOOLS.generate_data_dict(wanted_win_lst, fraction)
        lambda_num = len(lambda_dict)
        _value_per_win = _value / lambda_num
        for lambda_info_index, lambda_df in lambda_dict.items():
            lambda_info = wanted_win_lst[lambda_info_index]

            prev_lambda_info_index = lambda_info_index - 1
            if prev_lambda_info_index >= 0:
                prev_lambda_info = wanted_win_lst[prev_lambda_info_index]
                prev_columns = lambda_df.columns[lambda_df.columns.isin([prev_lambda_info])]
                lambda_df.loc[:, prev_columns] -= _value_per_win

            next_lambda_info_index = lambda_info_index + 1
            if next_lambda_info_index < len(wanted_win_lst):
                next_lambda_info = wanted_win_lst[next_lambda_info_index]
                next_columns = lambda_df.columns[lambda_df.columns.isin([next_lambda_info])]
                print(lambda_df.loc[:, next_columns])
                lambda_df.loc[:, next_columns] += _value_per_win
                print(lambda_df.loc[:, next_columns])

        lambda_unk_df = pd.concat(list(lambda_dict.values()))
        with bf_af_plot(output_file_dir, wanted_win_lst):
            large_df_to_csvs(lambda_unk_df, '.')

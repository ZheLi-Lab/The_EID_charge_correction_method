...
from ..common_tools.resample import RESAMPLE
import alchemlyb
...
class CAL_FREE_ENERGY():
    ...
    def __init__(self, u_nk_pd, wanted_win_lst=False, scale_f=0.75, force_align_mbar_list=True):
...
#             print('bbbb')
    ...
    def cal_FE_FEP(self, filename='free_ene_fep.csv', unit='kbT'):
        ...
        if self.lambda_range == 1:
            ...
            single_df = pd.DataFrame(columns=['lambda_value', 'FEP_forward_bythislambda(kbT)', 'FEP_reverse_bythislambda(kbT)','delta_what_to_what_forward', 'delta_what_to_what_reverse'], index =[0,] )
            ...
            if lambda_value == last_lambda_key:
                ...
                single_df.iloc[0,1] = esti_ene_0
                single_df.iloc[0,3] = np.NaN
                single_df.iloc[0,4] = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(first_lambda_key)
                ...
                single_df.iloc[0,2] = -esti_ene_0
                single_df.iloc[0,3] = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(last_lambda_key)
                single_df.iloc[0,4] = np.NaN
            else:
                esti_ene_0, esti_std_0 = self.cal_FE_first_window_bar()#use fep_backward calculation
                esti_ene_1, esti_std_1 = self.cal_FE_last_window_bar()#use fep_forward calculation
                single_df.iloc[0,1] = esti_ene_1
                single_df.iloc[0,2] = -esti_ene_0
                single_df.iloc[0,3] = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(self.simulation_lambda[i+1])
                single_df.iloc[0,4] = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(first_lambda_key)
            init_data_df = pd.concat([init_data_df, single_df], axis = 0 )
        else:
            for i in range(self.lambda_range):
                ...
                if i == 0:
                    if first_lambda_key in self.simulation_lambda.values():
                        ...
                        delta_what_to_what_forward = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(self.simulation_lambda[i+1])
                        delta_what_to_what_reverse = np.NaN
                    else:
                        fe_forward = self.cal_FE_FEP_forward(i)
                        fe_reverse = self.cal_FE_first_window_bar()[0]
                        delta_what_to_what_forward = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(self.simulation_lambda[i+1])
                        delta_what_to_what_reverse = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(first_lambda_key)
                    ...
                    if last_lambda_key in self.simulation_lambda.values():
                        ...
                        delta_what_to_what_forward = np.NaN
                        delta_what_to_what_reverse = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(self.simulation_lambda[i-1])
                    else:
                        fe_forward = self.cal_FE_last_window_bar()[0]
                        fe_reverse = -self.cal_FE_FEP_reverse(i)
                        delta_what_to_what_forward = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(last_lambda_key)
                        delta_what_to_what_reverse = tuple_to_str(self.simulation_lambda[i])+' to '+ tuple_to_str(self.simulation_lambda[i-1])
                else:
                    fe_forward = self.cal_FE_FEP_forward(i)
                    fe_reverse = -self.cal_FE_FEP_reverse(i)
                ...
                single_df['delta_what_to_what_forward'] = delta_what_to_what_forward
                single_df['delta_what_to_what_reverse'] = delta_what_to_what_reverse
                init_data_df = pd.concat([init_data_df,single_df], axis = 0)
    ...
    def cal_FE_first_window_bar(self, ifbootstrap_std=False):
        ...
        #print(f'first_key{first_key}', second_key)
